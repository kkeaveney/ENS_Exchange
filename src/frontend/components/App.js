...
const App = () => {
  ...
  const loadContracts = async () => {
    try {
      ...
      const mp = new ethers.Contract(EnsNameAddress.address, EnsNameAbi.abi, signer);
      setMarketplace(mp);
    ...
    <div>
      ...
      <div className="px-4 py-5 my-5">
        <Routes>
          ...
          <Route
            path="/buyens"
            element={<BuyEns marketplace={marketplace} ENSContract={ENSContract} account={account} />}></Route>
          <Route path="/search" element={<Search marketplace={marketplace} />}></Route>
        </Routes>
      </div>
    </div>
  );
